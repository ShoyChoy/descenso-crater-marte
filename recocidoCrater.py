...
class Recocidos(object):
    ...
    def neighbor(self):
        ...
        if discr[self.j + 1][self.i]==1:
            neighbors.append(Recocidos((self.j + 1,self.i)))
        
        if discr[self.j - 1][self.i]==1:
            neighbors.append(Recocidos((self.j - 1,self.i)))
                
        if discr[self.j][self.i + 1]==1:
            neighbors.append(Recocidos((self.j,self.i + 1)))
...
#P SE ESCRIBE YX
p = (1700, 1500)
recocido = Recocidos(p)        # Initialize search
cost = recocido.cost()             # Initial cost    
step = 0;                       # Step count


alpha = 0.99999; # Coefficient of the exponential temperature schedule        
...
while (t > 0.005) and (cost > b):
    ...
    if len(costos) == 0:
        ...
        r = random.randint(0,len(costos)-1)
        new_cost = costos[r]
    ...
    if new_cost < cost:
        ...
        pr = math.exp(-(new_cost - cost)/t)
        if pr >= random.random():
            recocido = neighbor[r]
            cost = new_cost
            
#    print(discr[recocido.j][recocido.i])
...
print('Ubicación final:', recocido.i, recocido.j)
print('Altura inicial;', image_data[p[0]][p[1]])
print('Altura final:', image_data[recocido.j][recocido.i])
print("--------Solution-----------")
#board.show_board()     
